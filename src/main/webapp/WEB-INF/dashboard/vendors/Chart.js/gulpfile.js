...
function bumpTask(complete) {
  util.log('Current version:', util.colors.cyan(package1.version));
  var choices = ['major', 'premajor', 'minor', 'preminor', 'patch', 'prepatch', 'prerelease'].map(function(versionType) {
    return versionType + ' (v' + semver.inc(package1.version, versionType) + ')';
  });
  inquirer.prompt({
    type: 'list',
    name: 'version',
    ...
    var increment = res.version.split(' ')[0],
      newVersion = semver.inc(package1.version, increment),
      oldVersion = package1.version;

    // Set the new versions into the bower/package object
    package1.version = newVersion;
    bower.version = newVersion;

    // Write these to their own files, then build the output
    fs.writeFileSync('package.json', JSON.stringify(package1, null, 2));
...
function releaseTask() {
  exec('git tag -a v' + package1.version);
}
